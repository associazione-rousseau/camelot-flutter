...
class BlogSearchProvider extends ChangeNotifier {
  ...
  void searchByAuthor(UserProfile userProfile) {
    isLoading = true;
    final Future<List<BlogInstantArticle>> future = getIt<BlogInstantArticleNetworkHandler>().getPosts(0, authorRousseauSlug: userProfile.slug);
    future.then((List<BlogInstantArticle> articles) {
      results = articles;
