...
class ResidenceContentBody extends StatefulWidget {
  ResidenceContentBody(this.currentUser) {
    ...
    divisionTextControllers.putIfAbsent(
        'country',
        () => TextEditingController(
            text: currentUser.country != null ? currentUser.country.name : ''));

    if (currentUser.regione != null)
    ...
    if (currentUser.provincia != null)
      selectedDivisions.putIfAbsent('provincia', () => currentUser.provincia);
    divisionTextControllers.putIfAbsent(
        'provincia',
        () => TextEditingController(
            text: currentUser.provincia != null
                ? currentUser.provincia.name
                : ''));

    if (currentUser.comune != null)
    ...
    if (currentUser.municipio != null)
      selectedDivisions.putIfAbsent('municipio', () => currentUser.municipio);
    divisionTextControllers.putIfAbsent(
        'municipio',
        () => TextEditingController(
            text: currentUser.municipio != null
                ? currentUser.municipio.name
                : ''));

    _overseasCityTextController =
        TextEditingController(text: currentUser.overseaseCity);
  }

  HashMap<String, TextEditingController> divisionTextControllers =
      HashMap<String, TextEditingController>();
  HashMap<String, ItalianGeographicalDivision> selectedDivisions =
      HashMap<String, ItalianGeographicalDivision>();
  TextEditingController _overseasCityTextController;
  CurrentUser currentUser;

  @override
  _ResidenceContentBodyState createState() => _ResidenceContentBodyState(
      currentUser,
      selectedDivisions,
      divisionTextControllers,
      _overseasCityTextController);
}

class _ResidenceContentBodyState extends State<ResidenceContentBody> {
  _ResidenceContentBodyState(this.currentUser, this.selectedDivisions,
      this.divisionTextControllers, this.overseasCityTextController);
  ...
  Widget build(BuildContext context) {
    ...
    if (currentUser.lastResidenceChangeRequest != null &&
        currentUser.lastResidenceChangeRequest.status == 'PENDING') {
      return Padding(
    ...
    for (String division in DIVISIONS) {
      if (selectedDivisions[division] != null) {
        ...
        if (division == 'municipio' &&
            (selectedDivisions['comune'].descendants == null ||
                selectedDivisions['comune'].descendants.isEmpty)) {
          continue;
        }
        visibleDivisions.add(division);
    ...
    if (divisionTextControllers['country'].text != 'Italy' &&
        (overseasCityTextController.text == null ||
            overseasCityTextController.text.isEmpty)) buttonEnabled = false;
    if (fieldsUnchanged(currentUser, selectedDivisions) == true)
      buttonEnabled = false;
            ...
            child: Padding(
              ...
              child: ListView.builder(
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    if (index == 0) {
                      return Padding(
                        ...
                        child: GeoAutocomplete(
                            'country',
                            divisionTextControllers['country'],
                              ...
                              children: <Widget>[
                                ...
                                Padding(
                                  padding: const EdgeInsets.only(
                                      top: 30, bottom: 10),
                                  child: Text(
                                    RousseauLocalizations.of(context)
                                        .text('last-italian-residence')
                                        .toUpperCase(),
                                    style:
                    ...
                    return visibleDivisions.contains(division)
                        ? GeoAutocomplete(
                            division,
                            divisionTextControllers[division],
                            _onSuggestionSelected,
                            selectedDivisions)
                        : Container();
                  }),
  ...
  void _onSuggestionSelected(
      String type, ItalianGeographicalDivision suggestion) {
    setState(() {
      ...
      DIVISIONS.asMap().forEach((int index, String value) {
        ...
        selectedDivisions.update(
            value, (ItalianGeographicalDivision value) => null);
