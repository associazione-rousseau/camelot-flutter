...
class SubscriptionsSheet<T extends HasList<Subscription>> extends StatelessWidget {
          ...
          return ClipRRect(
            ...
            child: Container(
              ...
              child: ChangeNotifierProvider<ListProvider<Subscription>>(
                create: (_) {
                  final GenericListProvider<T, Subscription> provider = GenericListProvider<T, Subscription>();
                  provider.onFetcherUpdated(fetcher);
                  return provider;
                },
                child: RousseauList<ListProvider<Subscription>, Subscription>(
                  primary: false,
                  itemBuilder: (BuildContext context, Subscription subscription) => UserRow(user: subscription.user),
                ),
              ),
            ),
          );
